...
void xmu_init()
{
    ...
    if (res == FR_EXIST)
    {
        ...
        MSC_BLOCK_NUM = f_size(&xmu_msc_file) / MSC_BLOCK_SIZE;
        TU_LOG1("%s already exists. File size set to %d\n", f_size(&xmu_msc_file));
    }
    else
    {
...
extern "C" int32_t tud_msc_read10_cb(uint8_t lun, uint32_t lba, uint32_t offset, void *buffer, uint32_t bufsize)
{
    ...
    f_read(&xmu_msc_file, buffer, bufsize, &br);
...
extern "C" int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba, uint32_t offset, uint8_t *buffer, uint32_t bufsize)
{
    ...
    f_write(&xmu_msc_file, buffer, bufsize, &bw);
