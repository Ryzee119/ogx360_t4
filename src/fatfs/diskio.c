...
)
{
    ...
    printf("[FATFS] Read from sector %i for %i sector(s)\n", sector, count);
...
)
{
    ...
    printf("[FATFS] Wrote to sector %i for %i sector(s)\n", sector, count);
    return RES_OK;
}
#endif
...
)
{
    ...
    case GET_SECTOR_SIZE:
        printf("[FATFS] disk_ioctl: Sector size %i\n", _disk_volume_get_block_size());
        *(WORD *)buff = _disk_volume_get_block_size();
        break;
    case GET_BLOCK_SIZE:
        printf("[FATFS] disk_ioctl: Block size %i\n", _disk_volume_get_cluster_size());
        *(DWORD *)buff = _disk_volume_get_cluster_size();
        break;
    case GET_SECTOR_COUNT:
        printf("[FATFS] disk_ioctl: Sector count %i\n", _disk_volume_num_blocks());
